...
public class MyFluentWait<T> implements Wait<T>
{
     ...
     */
    public MyFluentWait<T> withTimeout(long duration, TimeUnit unit)
    {
     ...
     * @return A self reference.
     */
    public MyFluentWait<T> pollingEvery(long duration, TimeUnit unit)
    {
...
//                    System.out.printf(">>> MFW returning value: %s\n", value.toString());
            ...
            if (!clock.isNowBefore(end))
            {
                ...
                String timeoutMessage = String.format("Expected condition failed: %s (tried for %d second(s) with %s interval)", message == null ? "waiting for " + isTrue : message, timeout.in(SECONDS), interval);
